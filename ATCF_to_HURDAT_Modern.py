...
data_tc = pd.read_csv("For storms.txt", usecols=range(11), header=None)
...
#data_tc2['time'] = data_tc2['time'].map('{:.2f}'.format)
data_tc2['time'] = data_tc2['time'].astype('string')
data_tc2['time'] = data_tc2['time'].str.zfill(2)
data_tc2['time'] = data_tc2['time'].str.ljust(width=4, fillchar='0')
